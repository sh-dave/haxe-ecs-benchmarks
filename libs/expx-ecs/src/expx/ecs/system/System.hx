...
@:autoBuild(expx.ecs.system.System.build())
...
class System {
	public static function build():Array<Field> {
				...
				case Success(_):
					...
					switch field.kind {
						case FVar(ct, e):
							...
							field.kind = FieldType.FVar(macro:expx.ecs.node.NodeList<$ct>, e);
		...
		function getEventType(type:Type) {
			...
			return (macro {
				function get<A>(s:expx.ecs.system.System<A>):A return null;
				get((null:$ct));
			}).typeof().sure();
		}

		var event = getEventType(Context.getLocalType()).toComplex();
		if(addedExprs.length > 0) builder.addMembers(macro class {override function setNodeLists(engine:expx.ecs.Engine<$event>) $b{addedExprs}});
